...
- 自动安装(推荐！)
    1. windows系统，下载install目录的 Hyperf_mall_service.exe到本地，双击运行，根据提示输入命令``99``执行一键安装。
    2. linux系统，下载install目录的python代码，执行 python ./main.py (python版本 > 3.0)，根据提示输入命令``99``执行一键安装。
    运行后如下图：
